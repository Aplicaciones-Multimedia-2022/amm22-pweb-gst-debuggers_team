...
if(evt.currentTarget.className != " active") {
	  myMusic.stop();
	  mySound.stop();
  } else if(evt.currentTarget.className == " active"){
	  myMusic.play();
  }
}
...
function init(){
	...
	//Pausar juego al pinchar pestaña
...
var vertical_oscilation = 20*(canvasWidth/1420)
...
var recorrer_backround = 0
	...
	function startgame(f_aparicion_met,numerodemet,vel_max_meteo){
		...
		astronauta = new Elemento(0.07*canvasWidth,getRndInteger(0.15*canvasHeight,0.66*canvasHeight),0,spriteElegido,20,escalaastro)
		
		for(let i = 0;i<numerodemet;i++){
			escalameteor = getRndInteger(1, 4)
			meteoritos[i] = new Elemento(canvasWidth,getRndInteger(50,600),getRndInteger((canvasWidth/1420),vel_max_meteo),spriteMeteorito,getRndInteger(2, 6),escalameteor);
	...
	function colisiones() {
		...
		for(let i=0;i<meteoritos.length;i++){
			...
			(		(distancia(astronauta.centroElementox1,astronauta.centroElementoy1,meteoritos[i].centroElementox,meteoritos[i].centroElementoy) <= 80/escalaastro))||
			(		(distancia(astronauta.centroElementox2,astronauta.centroElementoy2,meteoritos[i].centroElementox,meteoritos[i].centroElementoy) <= 70/escalaastro))){
				game_over();
			}
		}
		if(level >= 2){
			for(let i=0;i<ovnis.length;i++){
				...
				(		(distancia(astronauta.centroElementox1,astronauta.centroElementoy1,ovnis[i].centroElementox,ovnis[i].centroElementoy) <= 80/escalaastro))||
				(		(distancia(astronauta.centroElementox2,astronauta.centroElementoy2,ovnis[i].centroElementox,ovnis[i].centroElementoy) <= 70/escalaastro))){
					game_over();
				}
			}
		}
	...
	problem_vid.addEventListener('play',function() {
		drawvideo(problem_vid,ctx,canvasWidth, canvasHeight);
	});
	
	expl_vid.addEventListener('play',function() {
		drawvideo(expl_vid,ctx,canvasWidth, canvasHeight);
	});
